    ...
    public function addAction(){
        ...
        if (strtoupper($requestMethod) == 'POST') {      //POST
            try {
                ...
                $newClientId = $clientModel->add($name, $width, $height, $xPosition, $yPosition, $status, $times_used, $last_online, $joined_at);
            ...
            try {
                if (isset($arrParams['id']) && $arrParams['id']) {
                    ...
                    if (empty($dataToUpdate)) {
                        ...
                        $status = (is_array($arrParams) && isset($arrParams['status']) && $arrParams['status']) ? $arrParams['status'] 
                        : ((is_array($dataToUpdate) && isset($dataToUpdate[0]->status)) ? $dataToUpdate[0]->status 
                        : null);
                        ...
                        $result = $clientModel->updateThis($id, $name, $width, $height, $xPosition, $yPosition, $status, (int)$times_used, $last_online, $joined_at);
